...
import { allowFloatInput, allowCharacterNumberInput, allowCharacterNumberSpaceCommaInput, allowCharacterSpaceCommaInput } from '@/Components/Common/PowerUps/PowerupFunctions'
...
function ClusterPaymentCard(props) {

  const { propertyGenerateOrderId, propertyGenerateHoldingOrderId, api_getTransactionNoForReceipt, api_postPropertyPayment, api_postSafPayment } = CitizenApplyApiList();
  ...
  const makePayment = (data) => {
    ...
    axios.post(url, requestBody, ApiHeader())
      .then(function (response) {
        ...
        // return
        navigate(`/paymentReceipt/${response?.data?.transaction_no}/${moduleType}`)
        setisLoading(false)
      })
      .catch(function (error) {
        ...
        <div className="w-full h-full bg-white p-20">
          <div>
            ...
            <div className="text-center mt-6">
              ...
              {moduleType != "direct" && <button className={`mr-4 bg-white border border-indigo-500 text-indigo-500 px-4 py-1 shadow-lg hover:scale-105 rounded-sm`} onClick={() => navigate(`/propertyDashboard`)}>View Dashboard</button>}
            </div>
          </div>
        </div>
      </>
...
export default ClusterPaymentCard