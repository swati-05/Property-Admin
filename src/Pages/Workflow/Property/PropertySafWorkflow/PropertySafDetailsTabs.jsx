...
export default function PropertySafDetailsTabs(props) {
  ...
  const fetchDetailsById = () => {
    ...
    axios
      ...
      .then(function (response) {
        ...
        //* calling function to set custom tabs
  ...
  const renderCustomTabs = (currentRoleName) => {
    ...
    //* tabs for bo
    if (currentRoleName == safWorkflowCandidateNameKey?.BO) {
      setcurrentTabList(boTabs)
    }
    //* tabs for da
    if (currentRoleName == safWorkflowCandidateNameKey?.DA) {
      setcurrentTabList(daTabs)
    }
    //* tabs for si
    if (currentRoleName == safWorkflowCandidateNameKey?.SI) {
      setcurrentTabList(siTabs)
    }
    //* tabs for eo
    if (currentRoleName == safWorkflowCandidateNameKey?.EO) {
      setcurrentTabList(eoTabs)
    }
  ...
  const boTabs = [
    {
      label: 'Workflow', tab: <PropertySafWorkflowTimeline
        toast={notify}
    ...
    { label: 'Upload Documents', tab: <BoDocUpload id={index} applicationData={applicationData2} refresh={() => setrefresh(refresh + 1)} /> },
    { label: 'Communication', tab: <PropertySafCustomTab id={index} roleId={roleId} /> },
  ]
  const daTabs = [
    {
      label: 'Workflow', tab: <PropertySafWorkflowTimeline
    ...
    { label: 'Verify Documents', tab: <PropertySafDocumentVerify id={index} /> },
    { label: 'Communication', tab: <PropertySafCustomTab id={index} roleId={roleId} /> },
  ]
  const siTabs = [
    {
    ...
    { label: 'Documents', tab: <PropertySafDocumentView id={index} /> },
    { label: 'Communication', tab: <PropertySafCustomTab id={index} roleId={roleId} /> },
  ]
  const eoTabs = [
    {
            ...
            <Box sx={{ width: "100%" }}>
              <Box sx={{ borderBottom: 1, borderColor: "divider" }}>
                ...
                >
                 ...
                 <Tab label="Upload Documents" {...a11yProps(2)} />
                  <Tab label="Verify Documents" {...a11yProps(3)} />
                  <Tab label="Communication" {...a11yProps(4)} /> */}
                  {
                    currentTabList?.map((list, index) => (
                      <Tab label={list?.label} {...a11yProps(index)} />
                    ))
                  }
                </Tabs>
              </Box>
              {/* conditions tabs via tab array list */}
              {
                currentTabList?.map((list, index) => (
