...
function DeleteWorkflowRoleModal(props) {

  const {refresh, setrefresh} = useContext(contextVar)

  const [modalIsOpen, setIsOpen] = React.useState(false);

  const {wfRoleDelete} = apiList()
  ...
  const handleDelete = (id) => {
    ...
    axios
      ...
      .then(
        (res) => {
          toast.success("Workflow Role Deleted successfully !!")
          console.log("unique error getting => ", res);
      ...
      .catch(
        (err) => {
          toast.success("Failed to Deleted the Workflow Role !!")
...
export default DeleteWorkflowRoleModal;
