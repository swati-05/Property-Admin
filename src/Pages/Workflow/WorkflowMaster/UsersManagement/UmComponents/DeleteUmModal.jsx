...
function DeleteUmModal(props) {

  const {refresh, setrefresh} = useContext(contextVar)

  const {UmDelete} = apiList()
  ...
  const handleDelete = (id) => {
    ...
    axios
      ...
      .then(
        (res) => (toast.success("User Deleted successfully"), closeModal(),  setrefresh(refresh+1))
      )
      .catch(
        (err) => (toast.error("Failed to Deleted the User"), closeModal())
      );
  };
