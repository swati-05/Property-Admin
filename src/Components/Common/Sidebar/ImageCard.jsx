//////////////////////////////////////////////////////////////////////////////////////
//    Author - Talib Hussain
//    Version - 1.0
//    Date - 24 june 2022
//    Revision - 1
//    Project - JUIDCO
//    Component  - ImageCard
//    DESCRIPTION - ImageCard Component
//////////////////////////////////////////////////////////////////////////////////////
import { useState, useEffect,useContext } from 'react'
import { contextVar } from '@/Components/Context/Context';
import { FcDepartment } from 'react-icons/fc';
import { BsBuildings, BsBuildingsFill } from 'react-icons/bs';

function ImageCard(props) {
  const { userName, roles,setuserName,setroles } = useContext(contextVar)

  useEffect(() => {
    ///*** setting global data ***\\\\
    let lUser = userName==''? JSON.parse(window.localStorage.getItem('userName')):userName
    let lRoles = roles==''? JSON.parse(window.localStorage.getItem('roles')):roles
    console.log('username at sidebar...', lUser)
    console.log('roles at sidebar...', lRoles)
    setuserName(lUser)
    setroles(lRoles)

  }, [])

  return (
    <>
      <div className="text-center px-1" >
        {/* <img className='rounded-full m-auto mt-4' style={{ 'width': '58px' }} src="data:image/jpeg;base64," alt="image" /> */}
        <span className='text-[40px] flex justify-center py-2 text-white'><BsBuildings /></span>
        <p className='text-sm font-medium' style={props.sideBarStatus ? { 'display': 'none' } : { 'display': 'block' }}>{userName}</p>
        {roles !='' && roles?.map((role) => (
          <p className='text-xs' style={props.sideBarStatus ? { 'display': 'none' } : { 'display': 'block' }}>{role}</p>
        ))}
      </div>
    </>
  )
}

export default ImageCard
/**
 * Exported to :
 * 1. Sidebar Component
 */